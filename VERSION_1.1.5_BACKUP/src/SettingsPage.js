...
function SettingsPage() {
  ...
  // Overdue deadline settings
  const [showOverdueDeadlines, setShowOverdueDeadlines] = useState(true);
  const [overdueDaysLimit, setOverdueDaysLimit] = useState(90);
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      setShowOverdueDeadlines(userProfile.showOverdueDeadlines !== false); // Default to true
      setOverdueDaysLimit(userProfile.overdueDaysLimit || 90);
    }
  ...
  const handleOverdueDeadlinesToggle = async (enabled) => {
    try {
      const userRef = doc(db, 'users', userProfile.uid);
      await updateDoc(userRef, { showOverdueDeadlines: enabled });
      setShowOverdueDeadlines(enabled);
      setMessage('Overdue deadline settings updated successfully!');
      setTimeout(() => setMessage(''), 3000);
    } catch (error) {
      setError('Failed to update overdue deadline settings: ' + error.message);
    }
  };

  const handleOverdueDaysLimitChange = async (days) => {
    try {
      const userRef = doc(db, 'users', userProfile.uid);
      await updateDoc(userRef, { overdueDaysLimit: days });
      setOverdueDaysLimit(days);
      setMessage('Overdue days limit updated successfully!');
      setTimeout(() => setMessage(''), 3000);
    } catch (error) {
      setError('Failed to update overdue days limit: ' + error.message);
    }
  };
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-700 dark:text-gray-300 font-medium">Show Overdue Deadlines</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  When enabled, you'll see deadlines that are past their due date
                </p>
              </div>
              <button
                onClick={() => handleOverdueDeadlinesToggle(!showOverdueDeadlines)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-theme-primary focus:ring-offset-2 ${
                  showOverdueDeadlines ? 'bg-theme-primary' : 'bg-gray-200 dark:bg-gray-600'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
                    showOverdueDeadlines ? 'translate-x-6' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
            
            {showOverdueDeadlines && (
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-blue-800 dark:text-blue-200 mb-1">
                      Maximum Overdue Days to Show
                    </p>
                    <p className="text-xs text-blue-600 dark:text-blue-300">
                      Set to "No Limit" to see all overdue deadlines, or choose a specific number of days
                    </p>
                  </div>
                  <select
                    value={overdueDaysLimit === -1 ? 'no-limit' : overdueDaysLimit}
                    onChange={(e) => {
                      const value = e.target.value;
                      const days = value === 'no-limit' ? -1 : parseInt(value);
                      handleOverdueDaysLimitChange(days);
                    }}
                    className="px-3 py-2 border border-blue-200 dark:border-blue-600 rounded-lg bg-white dark:bg-gray-700 text-blue-800 dark:text-blue-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                ...
                <div className="mt-2 text-xs text-blue-600 dark:text-blue-300">
                  {overdueDaysLimit === -1 
                    ? 'Currently showing all overdue deadlines' 
                    : `Currently showing deadlines overdue by up to ${overdueDaysLimit} days`
                  }
                </div>
              </div>
            )}
