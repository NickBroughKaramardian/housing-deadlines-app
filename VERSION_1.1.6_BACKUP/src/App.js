...
function App() {
  ...
  const handleUpdateTask = async (updatedTask) => {
    ...
    const cleanUpdatedTask = Object.fromEntries(
      Object.entries(updatedTask).filter(([key, value]) => value !== undefined)
    );
  ...
  const getFilteredSortedTasks = () => {
    ...
    if (taskStatusFilter !== 'all') {
      ...
      // Apply overdue deadline settings only for specific status filters (not "all")
      if (userProfile && userProfile.showOverdueDeadlines === false) {
        const today = new Date();
        filtered = filtered.filter(task => {
          const deadlineDate = parseDeadlineDate(task.deadline);
          if (!deadlineDate) return true; // Keep tasks without deadlines
          const daysDiff = Math.ceil((deadlineDate.setHours(0,0,0,0) - today.setHours(0,0,0,0)) / 86400000);
          return daysDiff >= 0; // Only show non-overdue tasks
        });
      } else if (userProfile && userProfile.overdueDaysLimit && userProfile.overdueDaysLimit > 0) {
        const today = new Date();
        filtered = filtered.filter(task => {
          const deadlineDate = parseDeadlineDate(task.deadline);
          if (!deadlineDate) return true; // Keep tasks without deadlines
          const daysDiff = Math.ceil((deadlineDate.setHours(0,0,0,0) - today.setHours(0,0,0,0)) / 86400000);
          return daysDiff >= -userProfile.overdueDaysLimit; // Only show tasks within the limit
        });
      }
    }
