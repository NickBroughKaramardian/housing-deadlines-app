...
function BatchTaskForm({ addTask }) {
  ...
  const tableHeaders = [
    'Project', 'Description', 'Deadline', 'Responsible (comma-separated)', 'Recurring', 'Interval (months)', 'Final Year', 'Notes', 'Document Link', ''
  ];
  ...
  const handleTablePaste = (e) => {
    ...
    const fieldMap = ['projectName', 'description', 'deadline', 'responsibleParty', 'recurring', 'frequency', 'finalYear', 'notes', 'documentLink'];
  ...
  const handleTableSubmit = (e) => {
    ...
    tableRows.forEach((row, idx) => {
      ...
      const finalYear = (row.finalYear || '').trim();
      ...
      if (!projectName && !description && !deadline && !responsibleParty && !frequency && !finalYear && !recurringRaw) return;
                ...
                <tbody>
                  {tableRows.map((row, idx) => (
                    <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td><input type="text" value={row.finalYear} onChange={e => handleTableChange(idx, 'finalYear', e.target.value)} className="border-none px-2 py-2 w-full bg-transparent text-center focus:outline-none text-xs" onKeyDown={handleInputKeyDown} onFocus={handleInputFocus} onBlur={handleInputBlur} placeholder="Final Year" /></td>
