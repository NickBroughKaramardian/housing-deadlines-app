...
export default function UserManagement() {
  ...
  const handleDepartmentChange = async (userId, departmentId, isAdding) => {
    try {
      ...
      if (isAdding) {
        ...
        newDepartments = currentDepartments.filter(dept => dept !== departmentId);
      }

      await updateUserDepartments(userId, newDepartments);
      setUsers(users.map(user => 
        user.id === userId ? { ...user, departments: newDepartments } : user
      ));
    } catch (error) {
      console.error('Error updating user departments:', error);
      alert('Error updating user departments: ' + error.message);
    }
  };
        ...
        <div className="space-y-8">
          ...
          <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-3">
              {users.map((user) => (
                <div key={user.id} className="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {Object.entries(DEPARTMENTS).map(([key, deptId]) => {
                          const isInDept = isUserInDepartment(user, deptId);
                          return (
                            <button
                              key={deptId}
                              onClick={() => handleDepartmentChange(user.id, deptId, !isInDept)}
                              disabled={user.id === userProfile.uid && userProfile.role !== ROLES.DEVELOPER}
                              ...
                              title={`${isInDept ? 'Remove from' : 'Add to'} ${DEPARTMENT_NAMES[deptId]}`}
                            >
