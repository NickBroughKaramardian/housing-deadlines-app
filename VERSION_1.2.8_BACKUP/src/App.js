...
function App() {
  ...
  const handleAddTask = async (newTask) => {
    ...
    const autoDepartment = getDepartmentFromResponsibleParty(newTask.responsibleParty, departmentMappings);
  ...
  const handleUpdateTask = async (updatedTask) => {
    ...
    const autoDepartment = getDepartmentFromResponsibleParty(updatedTask.responsibleParty, departmentMappings);
  ...
  const handleImportTasks = async (importedTasks) => {
    for (const task of importedTasks) {
      // Auto-detect department based on responsible party
      const autoDepartment = getDepartmentFromResponsibleParty(task.responsibleParty, departmentMappings);
      ...
      <div className="hidden lg:flex items-center justify-between px-4 py-4 bg-white border-b border-gray-100 shadow-sm sticky top-0 z-20">
        ...
        <div className="flex items-center gap-4 w-full max-w-7xl mx-auto">
          <div className="flex flex-1 gap-2 w-full">
            ...
            {/* Deadlines Dropdown */}
            <div className="relative view-deadlines-dropdown flex-1">
              <button 
                onClick={() => setViewDeadlinesOpen(!viewDeadlinesOpen)}
            ...
            {/* Admin Dropdown */}
            <div className="relative admin-dropdown flex-1">
              <button 
                onClick={() => setAdminMenuOpen(!adminMenuOpen)}
                className={`w-full px-4 py-2 rounded text-center flex items-center justify-center gap-2 ${
                  ['data', 'users', 'settings'].includes(activeTab) 
