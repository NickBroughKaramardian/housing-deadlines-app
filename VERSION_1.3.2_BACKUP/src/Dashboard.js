...
function Dashboard({ tasks, users, departmentMappings, onToggleCompleted, onTaskLinkClick }) {
  const { 
    userProfile, 
    hasPermission, 
    ROLES, 
  ...
  const getDepartmentProgress = () => {
    ...
    Object.values(DEPARTMENTS).forEach(deptId => {
      ...
      const deptTasks = tasks.filter(task => {
        ...
        // First, check if the responsible party matches the department name directly
        const autoDepartment = getDepartmentFromResponsibleParty(task.responsibleParty, departmentMappings || []);
        if (autoDepartment === deptId) {
          return true;
        }
        
        // Then, check if task is assigned to someone in this department
