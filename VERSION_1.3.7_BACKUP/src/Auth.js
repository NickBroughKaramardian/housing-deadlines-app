...
// Function to auto-detect department based on responsible party
export const getDepartmentFromResponsibleParty = (responsibleParty, customMappings = [], users = [], aliases = []) => {
  ...
  const customMapping = customMappings.find(mapping => 
    mapping.term?.toLowerCase() === party || 
    party.includes(mapping.term?.toLowerCase())
  );
  if (customMapping) {
    return customMapping.department;
  }
  
  // Default department keyword mappings
  if (party === 'development' || party === 'dev') {
    return DEPARTMENTS.DEVELOPMENT;
  }
  if (party === 'accounting' || party === 'accountant' || party === 'finance') {
    return DEPARTMENTS.ACCOUNTING;
  }
  if (party === 'compliance' || party === 'legal') {
    return DEPARTMENTS.COMPLIANCE;
  }
  if (party === 'management' || party === 'manager' || party === 'admin') {
    return DEPARTMENTS.MANAGEMENT;
  }
  
  // Keyword contains checks
  if (party.includes('development') || party.includes('dev')) {
    return DEPARTMENTS.DEVELOPMENT;
  }
  if (party.includes('accounting') || party.includes('finance') || party.includes('accountant')) {
    return DEPARTMENTS.ACCOUNTING;
  }
  if (party.includes('compliance') || party.includes('legal')) {
    return DEPARTMENTS.COMPLIANCE;
  }
  if (party.includes('management') || party.includes('manager') || party.includes('admin')) {
    return DEPARTMENTS.MANAGEMENT;
  }

  // Flexible person-name resolution using aliases and users
  const tokens = party.split(',').map(t => t.trim()).filter(Boolean);

  const normalize = (s) => s.toLowerCase().trim();

  const resolveTokenToUsers = (token) => {
    const t = normalize(token);
    const directAliasMatches = aliases
      .filter(a => normalize(a.term) === t)
      .map(a => users.find(u => u.uid === a.userId))
      .filter(Boolean);
    if (directAliasMatches.length > 0) return directAliasMatches;

    // Exact displayName/email
    const exact = users.filter(u => normalize(u.displayName || '') === t || normalize(u.email || '') === t);
    if (exact.length > 0) return exact;

    // First-name prefix or token substring within displayName words (unique only)
    const candidates = users.filter(u => {
      const dn = normalize(u.displayName || '');
      if (!dn) return false;
      const words = dn.split(/\s|-/).filter(Boolean);
      return words.length > 0 && (words[0].startsWith(t) || words.some(w => w === t || w.startsWith(t)));
    });
    return candidates;
  };

  const matchedUsers = tokens.flatMap(resolveTokenToUsers);
  const deptIds = new Set();
  matchedUsers.forEach(u => {
    if (Array.isArray(u?.departments)) {
      u.departments.forEach(d => deptIds.add(d));
    }
  });

  // If exactly one department inferred across all resolved users, return it
  if (deptIds.size === 1) {
    return Array.from(deptIds)[0];
  }

  return null;
};

// Function to get all department mappings
export const getDepartmentMappings = async (organizationId) => {
  try {
    const mappingsRef = collection(db, 'departmentMappings');
    const q = query(mappingsRef, where('organizationId', '==', organizationId));
...
// Function to add a department mapping
export const addDepartmentMapping = async (mapping) => {
  try {
    const mappingsRef = collection(db, 'departmentMappings');
...
// Function to update a department mapping
export const updateDepartmentMapping = async (mappingId, updates) => {
  try {
    const mappingRef = doc(db, 'departmentMappings', mappingId);
...
// Function to delete a department mapping
export const deleteDepartmentMapping = async (mappingId) => {
  try {
    const mappingRef = doc(db, 'departmentMappings', mappingId);
