...
function DataManagement({ tasks, overrides, onImportTasks, onRestoreBackup, users = [], aliases = [], departmentMappings = [] }) {
  ...
  const getDepartmentBreakdown = () => {
    ...
    expanded.forEach(task => {
      ...
      // Alias-aware auto department
      const autoDept = getDepartmentFromResponsibleParty(task.responsibleParty, departmentMappings || [], users || [], aliases || []);
      if (autoDept && byDept[autoDept]) {
        byDept[autoDept].total += 1;
        if (task.completed) byDept[autoDept].completed += 1;
        return;
      }
      ...
      if (!matched) {
        ...
        if (p.includes('development')) {
          byDept[DEPARTMENTS.DEVELOPMENT].total += 1; if (task.completed) byDept[DEPARTMENTS.DEVELOPMENT].completed += 1; return;
        }
        if (p.includes('accounting') || p.includes('finance') || p.includes('accountant')) {
          byDept[DEPARTMENTS.ACCOUNTING].total += 1; if (task.completed) byDept[DEPARTMENTS.ACCOUNTING].completed += 1; return;
        }
        if (p.includes('compliance') || p.includes('legal')) {
          byDept[DEPARTMENTS.COMPLIANCE].total += 1; if (task.completed) byDept[DEPARTMENTS.COMPLIANCE].completed += 1; return;
        }
        if (p.includes('management') || p.includes('manager') || p.includes('admin')) {
          byDept[DEPARTMENTS.MANAGEMENT].total += 1; if (task.completed) byDept[DEPARTMENTS.MANAGEMENT].completed += 1; return;
        }
        byDept['unassigned'].total += 1;
        if (task.completed) byDept['unassigned'].completed += 1;
      }
  ...
  return (
    <div className="space-y-6">
      {/* Year Filter moved below metrics */}
