...
function Dashboard({ tasks, users, departmentMappings, onToggleCompleted, onTaskLinkClick, aliases = [] }) {
  ...
  const getDepartmentProgress = () => {
    ...
    // Helper: resolve a token (e.g., "Nick") to one or more users using aliases and direct matches
    const normalize = (s) => (s || '').toLowerCase().trim();
    const tokenToUsers = (token) => {
    ...
    Object.values(DEPARTMENTS).forEach(deptId => {
      ...
      const deptTasks = tasks.filter(task => {
        ...
        if (deadlineDate.getFullYear() !== currentYear) return false;
        if (!task.responsibleParty) return false;

        // 1) Alias-aware auto department
        const autoDepartment = getDepartmentFromResponsibleParty(task.responsibleParty, departmentMappings || [], users || [], aliases || []);
        if (autoDepartment === deptId) return true;

        // 2) Resolve each token to users and check if any resolved user is in this department
        const tokens = task.responsibleParty.split(',').map(t => t.trim()).filter(Boolean);
        for (const tok of tokens) {
          const matchedUsers = tokenToUsers(tok);
          if (matchedUsers.some(u => Array.isArray(u?.departments) && u.departments.includes(deptId))) {
            return true;
          }
        }

        // 3) Fallback: department keyword in token (e.g., "Development")
        const p = normalize(task.responsibleParty);
        if (deptId === DEPARTMENTS.DEVELOPMENT && p.includes('development')) return true;
        if (deptId === DEPARTMENTS.ACCOUNTING && (p.includes('accounting') || p.includes('finance') || p.includes('accountant'))) return true;
        if (deptId === DEPARTMENTS.COMPLIANCE && (p.includes('compliance') || p.includes('legal'))) return true;
        if (deptId === DEPARTMENTS.MANAGEMENT && (p.includes('management') || p.includes('manager') || p.includes('admin'))) return true;
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 min-h-screen w-full min-h-0">
      ...
      <div className="w-full lg:w-2/3 flex flex-col min-h-0 flex-1">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 lg:p-6 flex-1 flex flex-col min-h-0 h-full overflow-x-hidden">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4 flex items-center">
          ...
          <div className="flex-1 min-h-0">
            {tasksDueThisWeek.length === 0 ? (
              ...
              <div className="space-y-4 h-full overflow-y-auto overflow-x-hidden px-2">
                {tasksDueThisWeek.map((task) => {
                  ...
                  return (
                    <div key={task.id} className="relative overflow-visible p-1">
                      <div
                        className={`group relative rounded-xl border ${status.border} ${status.bg} transition-all duration-300 hover:shadow-lg hover:scale-[1.02] hover:border-opacity-80`}
                        style={{ outline: '1px solid transparent', outlineOffset: 0, transformOrigin: 'center' }}
                      >
                        {/* Background pattern */}
      ...
      <div className="w-full lg:w-1/3 flex flex-col gap-4 lg:gap-6 min-h-0">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 lg:p-6 flex-1 flex flex-col">
          ...
          {/* Notes List */}
          <div className="flex-1 space-y-3">
            {(notes.length === 0 && teamAnnouncements.length === 0) ? (
            ...
            {(() => {
              ...
              return Object.entries(departmentStats).map(([deptId, stats]) => (
                <div key={deptId} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                  ...
                  <div className="flex justify-start text-xs text-gray-500 dark:text-gray-400">
