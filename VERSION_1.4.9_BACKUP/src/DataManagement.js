...
import { DEPARTMENTS, getDepartmentFromResponsibleParty } from './Auth';
import { 
  ArrowDownTrayIcon, 
  ArrowUpTrayIcon, 
...
function DataManagement({ tasks, overrides, onImportTasks, onRestoreBackup, users = [], aliases = [], departmentMappings = [] }) {
  ...
  // Current year for breakdown filtering and labeling
  const currentYear = new Date().getFullYear();

  // Build Department breakdown consistent with Dashboard logic (alias-aware)
  const getDepartmentBreakdown = () => {
    ...
    expanded.forEach(task => {
      ...
      if (task.responsibleParty) {
        // 1) Alias-aware auto department
        const autoDept = getDepartmentFromResponsibleParty(task.responsibleParty, departmentMappings || [], users || [], aliases || []);
        if (autoDept && Object.values(DEPARTMENTS).includes(autoDept)) {
          matchedDepts.add(autoDept);
        }

        // 2) Resolve tokens to users, collect their departments
        ...
        tokens.forEach(tok => {
          ...
          (matchedUsers || []).forEach(u => {
            if (Array.isArray(u?.departments)) {
              u.departments.forEach(did => {
                if (Object.values(DEPARTMENTS).includes(did)) matchedDepts.add(did);
              });
            }
          });
        });

        // 3) Department keyword fallback
        const p = normalize(task.responsibleParty);
        if (p.includes('development')) matchedDepts.add(DEPARTMENTS.DEVELOPMENT);
        if (p.includes('accounting') || p.includes('finance') || p.includes('accountant')) matchedDepts.add(DEPARTMENTS.ACCOUNTING);
        if (p.includes('compliance') || p.includes('legal')) matchedDepts.add(DEPARTMENTS.COMPLIANCE);
        if (p.includes('management') || p.includes('manager') || p.includes('admin')) matchedDepts.add(DEPARTMENTS.MANAGEMENT);
      }

      if (matchedDepts.size === 0) {
        byDept['unassigned'].total += 1;
        if (task.completed) byDept['unassigned'].completed += 1;
      } else {
        matchedDepts.forEach(did => {
          byDept[did].total += 1;
          if (task.completed) byDept[did].completed += 1;
        });
      }
    });
  ...
  return (
    <div className="space-y-6">
      {/* Year filter removed; using current year in labels and calculations */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Department Breakdown ({currentYear})</h3>
        ...
        {/* Project Breakdown */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Project Breakdown ({currentYear})</h3>
          <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
