...
function SettingsPage() {
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      // Load aliases for this user
      loadAliases();
    }
  }, [userProfile, hasPermission, ROLES.DEVELOPER]);

  // Sync theme/dark mode UI with themeService
  useEffect(() => {
  ...
  const loadAliases = async () => {
    ...
    try {
      ...
      const snap = await getDocs(qAliases);
      setAliases(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    } catch (e) {
  ...
  const addAlias = async () => {
    const term = (newAlias || '').trim();
    if (!term) return;
    try {
      const aliasesRef = collection(db, 'nameAliases');
      await addDoc(aliasesRef, {
        term,
      ...
      alert('Error adding nickname: ' + e.message);
    }
  };

  const removeAlias = async (id) => {
    try {
      await deleteDoc(doc(db, 'nameAliases', id));
      await loadAliases();
    } catch (e) {
      alert('Error removing nickname: ' + e.message);
    }
  };
    ...
    <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 lg:p-8">
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">Add the names people commonly use for you (e.g., "Nick"). These help match tasks to your profile for department reports.</p>
          ...
          <div className="flex gap-2 mb-4">
            <input
              ...
              className="flex-1 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-theme-primary focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
            />
            <button
              onClick={addAlias}
          ...
          {aliases.length === 0 ? (
            <div className="text-sm text-gray-500 dark:text-gray-400">No nicknames added yet.</div>
          ) : (
            <div className="flex flex-wrap gap-2">
              {aliases.map(a => (
                <span key={a.id} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-gray-800 dark:text-gray-200">
                  {a.term}
                  <button onClick={() => removeAlias(a.id)} className="text-red-600 dark:text-red-400 hover:underline text-xs">Remove</button>
                </span>
              ))}
            </div>
          )}
