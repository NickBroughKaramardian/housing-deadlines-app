...
function App() {
  ...
  const handleAddTask = async (newTask) => {
    ...
    try {
      ...
      console.log('Task added successfully with ID:', result.id);
    } catch (error) {
  ...
  const handleImportTasks = async (importedTasks) => {
    for (const task of importedTasks) {
      ...
      await microsoftDataService.tasks.add(taskWithOrg);
    }
  };
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-gray-100 shadow-sm">
          <div className="flex flex-col p-2 gap-1">
            ...
            <button onClick={() => { setActiveTab('database'); setIsMobileMenuOpen(false); }} className={`w-full px-4 py-3 rounded-lg text-left font-medium ${activeTab === 'database' ? 'bg-theme-primary text-white' : 'bg-gray-100 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300'}`}>Database</button>
      ...
      <div className="hidden lg:flex items-center justify-between px-4 py-4 bg-white border-b border-gray-100 shadow-sm sticky top-0 z-20">
        ...
        <div className="flex items-center gap-4 w-full max-w-7xl mx-auto">
          <div className="flex flex-1 gap-2 w-full">
            ...
            <button onClick={() => setActiveTab('database')} className={`flex-1 px-4 py-2 rounded text-center ${activeTab === 'database' ? 'bg-theme-primary text-white' : 'bg-gray-200 hover:bg-gray-400 dark:bg-gray-700 dark:hover:bg-gray-600'}`}>Database</button>
