...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const unsubReceived = onSnapshot(receivedMessagesQuery, (receivedSnapshot) => {
      ...
      const unsubSent = onSnapshot(sentMessagesQuery, (sentSnapshot) => {
        ...
        // Add 250ms delay before hiding loading screen
        setTimeout(() => {
  ...
  const handleDeleteMessage = async (id) => {
    try {
      ...
      // If it's an announcement and user has admin/owner/developer permissions, delete for everyone
      if (message.isAnnouncement && (hasPermission(ROLES.ADMIN) || hasPermission(ROLES.OWNER) || hasPermission(ROLES.DEVELOPER))) {
      ...
      // Otherwise: per-user soft delete
  ...
  const handleComposeSend = async (e) => {
    ...
    return (
    <div className="max-w-6xl mx-auto min-h-[600px] relative">
      {/* Loading Overlay */}
      {showLoading && (
