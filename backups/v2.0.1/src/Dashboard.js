...
function Dashboard({ tasks: propTasks, users, departmentMappings, onToggleCompleted, onTaskLinkClick, aliases = [] }) {
  ...
  // Load tasks directly from shared data service
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        setTasks(tasksData);
        console.log('Dashboard: Loaded', tasksData.length, 'tasks from shared service');
        
        // Debug: Log sample task data to understand structure
        if (tasksData.length > 0) {
          console.log('Dashboard: Sample task data:', {
            firstTask: tasksData[0],
            taskFields: Object.keys(tasksData[0])
          });
          
          // Debug: Check all tasks for overdue status
          console.log('Dashboard: Checking all tasks for overdue status...');
          console.log('Dashboard: All task statuses:', tasksData.map(task => ({
            id: task.id,
            Task: task.Task,
            Status: task.Status,
            status: task.status
          ...
          tasksData.forEach((task, index) => {
            ...
            console.log(`Dashboard: Task ${index}:`, {
              id: task.id,
              Task: task.Task,
              ...
              Completed_x003f_: task.Completed_x003f_,
  ...
  function parseDeadlineDate(dateStr) {
    ...
    try {
      // First try to parse as ISO string (what we're getting from SharePoint)
  ...
  // Calculate top metrics using correct SharePoint fields
  const getTopMetrics = () => {
  ...
  const tasksThisWeek = tasks.filter(task => {
    ...
    // Count overdue tasks using calculated status
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 pb-16">
                ...
                {tasksThisWeek.map(task => (
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <h4 className={`font-medium text-sm ${task.isCompleted ? 'text-green-800 dark:text-green-200 line-through' : 'text-gray-900 dark:text-white'}`}>
                      ...
                      <div className="flex items-center justify-between">
                        <p className={`text-xs font-medium ${task.isCompleted ? 'text-green-600 dark:text-green-400' : 'text-gray-900 dark:text-white'}`}>
                          {task.deadline ? format(task.deadline, 'MMM dd') : 'No date'}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {task.daysUntil === 0 ? 'Today' : 
                           ...
                           `${Math.abs(task.daysUntil)} days ago`}
                        </p>
                      </div>
                    </div>
                  </div>
