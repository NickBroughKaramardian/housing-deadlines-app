...
const Database = () => {
  ...
  const updateCell = async (taskId, field, value) => {
    ...
    try {
      ...
      globalTaskStore.updateTask(taskId, { [field]: processedValue });
      ...
      <div className="w-full px-4 py-8">
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            {hasPermission(userProfile?.role, 'ADMIN') && (
              <button 
                onClick={addNewRow} 
                    ...
                    {columns.map((column) => {
                      ...
                      return (
                        <td 
                          ...
                          style={{ width: column.width, minWidth: column.width }}
                          onClick={(e) => hasPermission(userProfile?.role, 'ADMIN') && column.key !== 'Status' && handleCellClick(task.id, column.key, e)}
                          onDoubleClick={() => hasPermission(userProfile?.role, 'ADMIN') && handleCellDoubleClick(task.id, column.key)}
                        >
                          {isEditing ? (
                            <div className="w-full">
                              {column.type === 'select' ? (
                                ...
                                <input 
                                  ...
                                  onBlur={(e) => { console.log("DEBUG: onBlur task.id:", task.id, "column.key:", column.key); updateCell(task.id, column.key, e.target.value); }} 
                                  className="w-full px-2 py-1 border rounded bg-white dark:bg-gray-700 text-sm" 
                                  autoFocus 
                                />
                              ) : column.type === 'url' ? (
                            ...
                            <div className="w-full">
                              {column.key === 'ResponsibleParty' ? (
                                ...
                                <span className="text-gray-700 dark:text-gray-300 text-sm break-all">
                                  {typeof task.Link === 'object' && task.Link !== null ? (task.Link.Url || task.Link.Description || '') : task.Link}
                                </span>
                              ) : (
