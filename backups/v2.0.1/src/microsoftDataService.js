  ...
  tasks: {
    ...
    update: async (taskId, updates) => {
      try {
        ...
        if (updates.Task) sharePointFields.Title = updates.Task;
        if (updates.Project) sharePointFields.Project = updates.Project;
        if (updates.Deadline) sharePointFields.Deadline = updates.Deadline;
        
        // Format ResponsibleParty for SharePoint REST API
        // REST API uses ResponsiblePartyId field with array of IDs
        if (updates.ResponsibleParty) {
        ...
        sharePointFields['Completed_x003f_'] = updates.Completed ? 'Yes' : 'No';
        
        if (updates.Interval) sharePointFields.Interval = updates.Interval;
        if (updates.FinalDate) sharePointFields.FinalDate = updates.FinalDate;
        
        sharePointFields.Priority = (updates.Priority === 'Urgent') ? 'Urgent' : 'Normal';
        
        if (updates.Notes) sharePointFields.Notes = updates.Notes;
        if (updates.Link) sharePointFields.Link = updates.Link;
